...
class ScatterPlot extends PureComponent {
  ...
  xAxis = d3
    ...
    .tickValues(d3.range(100, this.props.data.length, 100));
  yAxis = d3
    .axisLeft(this.state.yScale)
    .tickValues(d3.range(100, d3.max(this.props.data, d => d), 100));
            ...
            <g>
              {this.props.data.map((d, i) => (
                <circle
                  key={i}
                  r="5"
                  cx={this.state.xScale(i)}
                  cy={this.state.yScale(d)}
                  className="scatterplot1000"
                />
              ))}
            </g>
